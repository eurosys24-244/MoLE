...
from mole.utils import mole_checkoverflow, mole_setoverflow, mole_syncoverflow
from mole.utils import timer_cpu_start, timer_cpu_end, timer_cuda_start, timer_cuda_end
...
class DeepSpeedZeroOptimizer(ZeROOptimizer):
    ...
    def copy_grads_in_partition(self, param):
        if self.cpu_offload:
            ...
            if self.is_gradient_accumulation_boundary:
                ...
                timer_cuda_start("offload")
                self.async_inplace_copy_grad_to_fp32_buffer_from_gpu(param)
                timer_cuda_end("offload")
            return
    ...
    def reduce_ipg_grads(self):
        ...
        torch.cuda.nvtx.range_push("offload")
        with get_accelerator().stream(stream):
    ...
    def step(self, closure=None):
        ...
        for i, group in enumerate(self.bit16_groups):
            ...
            if self.cpu_offload:
                ...
                timer_cpu_start("real-step")
                self._optimizer_step(i)
                timer_cpu_end("real-step")
                from deepspeed.ops.adam import DeepSpeedCPUAdam
                if not (type(self.optimizer) == DeepSpeedCPUAdam
                        and self.dtype == torch.half):
                    bit16_partitions = self.parallel_partitioned_bit16_groups[i]
                    fp32_partition = self.single_partition_of_fp32_groups[i]
                    timer_cuda_start("copy-back")
                    bit16_partitions[partition_id].data.copy_(fp32_partition.data)
                    timer_cuda_end("copy-back")
                    #to correctly measure 
                    #print("copy back: ", self.dtype, bit16_partitions[partition_id].device, fp32_partition.data.device) #fp16 training
                ...
                self.start_timers([OPTIMIZER_STEP])
                timer_cuda_start("real-step")
                self._optimizer_step(i)
                timer_cuda_end("real-step")
    ...
    def _check_overflow(self, partition_gradients=True):
        ...
        self.overflow = self.overflow or mole_checkoverflow()
        if mole_syncoverflow():
            overflow_detect = torch.tensor([self.overflow]).cuda()
            torch.distributed.all_reduce(overflow_detect)
            mole_setoverflow(overflow_detect[0].item())
            self.overflow = overflow_detect[0].item()
        else:
            mole_setoverflow(self.overflow)
