...
class FusedExpertsNetwork(torch.nn.Module):
    def __init__(self, hidden_size_per_expert, activation_fn=None, activation_fn_with_self=None, output_dim=None, ctx=None):
        super().__init__()
    ...
    def update(self, ctx):
        ...
        if ctx.emulate_mole:
            self.cpu_batched_fc1_w = torch.empty_like(self.batched_fc1_w, device="cpu")
            self.cpu_batched_fc2_w = torch.empty_like(self.batched_fc2_w, device="cpu")
            with torch.no_grad():
                self.cpu_batched_fc1_w.copy_(self.batched_fc1_w)
                self.cpu_batched_fc2_w.copy_(self.batched_fc2_w)
    ...
    def forward(self, x, ctx):
        ...
        if ctx.emulate_mole:
            with torch.no_grad():
                batched_fc1_w.data.copy_(self.cpu_batched_fc1_w.data)
                batched_fc2_w.data.copy_(self.cpu_batched_fc2_w.data)
