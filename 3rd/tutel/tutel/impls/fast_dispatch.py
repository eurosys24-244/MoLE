...
class GatingEncoder(torch.autograd.Function):
    ...
    def forward(ctx: Any, config: Any, reshaped_input: Tensor, *gates_):
        ...
        for g, i, l in zip(ctx.gates_h2, ctx.config.indices_, ctx.config.locations_):
          ctx.config.func_fwd(g, i, l, reshaped_input, dispatched_input, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim, capacity])
    ...
    def backward(ctx: Any, dispatched_input: Tensor):
        ...
        for g, i, l in zip(ctx.gates_h2, ctx.config.indices_, ctx.config.locations_):
          ...
          ctx.config.func_bwd_data(g, i, l, grad_data, dispatched_input, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim, capacity])
        ...
        if id(ctx.gates_h2[0]) != id(ctx.config.ones_helper):
          for i, l in zip(ctx.config.indices_, ctx.config.locations_):
            ...
            ctx.config.func_bwd_gate(grad_gates1_s, i, l, ctx.reshaped_input, dispatched_input, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim, capacity])
...
class GatingDecoder(torch.autograd.Function):
    ...
    def forward(ctx: Any, config: Any, expert_output: Tensor, *gates_):
        ...
        for g, i, l in zip(ctx.gates_h2, ctx.config.indices_, ctx.config.locations_):
          ...
          config.func_bwd_data(g, i, l, single_output, expert_output, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim, capacity])
    ...
    def backward(ctx: Any, combined_output: Tensor):
        ...
        if ctx.config.cf0:
            ...
            capacity = ctx.config.capacity
        for g, i, l in zip(ctx.gates_h2, ctx.config.indices_, ctx.config.locations_):
          ctx.config.func_fwd(g, i, l, combined_output, grad_expert_output, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim, capacity])

        grad_gates = []
        if id(ctx.gates_h2[0]) != id(ctx.config.ones_helper):
          for i, l in zip(ctx.config.indices_, ctx.config.locations_):
            grad_gates1_s = torch.empty([ctx.config.sample_size,], dtype=combined_output.dtype, device=combined_output.device)
            ctx.config.func_bwd_gate(grad_gates1_s, i, l, combined_output, ctx.expert_output, extra=[ctx.config.indices_[0].size(0), ctx.config.aligned_dim, capacity])
            grad_gates.append(grad_gates1_s)
...
class TutelMoeFastDispatcher:
    ...
    def __init__(self, num_global_experts, capacity, model_dim, dispatch_dtype, cf0 = False):
    ...
    def update(self, indices_, locations_, gates_, capacity=None, is_postscore=True, is_tight=False):
        ...
        if self.is_cuda != indices_[0].is_cuda or self.is_tight != is_tight:
            self.is_cuda = indices_[0].is_cuda
            self.is_tight = is_tight
            if (self.is_cuda, self.is_tight) not in TutelMoeFastDispatcher.kernel_pool:
                self.func_fwd = jit_kernel.create_forward(self.dtype, indices_[0].is_cuda, is_tight)
                self.func_bwd_data = jit_kernel.create_backward_data(self.dtype, indices_[0].is_cuda, is_tight)
                self.func_bwd_gate = jit_kernel.create_backward_gate(self.dtype, indices_[0].is_cuda, is_tight)
                TutelMoeFastDispatcher.kernel_pool[(self.is_cuda, self.is_tight)] = self.func_fwd, self.func_bwd_data, self.func_bwd_gate
            else:
                self.func_fwd, self.func_bwd_data, self.func_bwd_gate = TutelMoeFastDispatcher.kernel_pool[(self.is_cuda, self.is_tight)]
...
def extract_critical(scores, top_k, loss_fn=losses.gshard_loss, capacity_factor=1.0, batch_prioritized_routing=False, normalize_gate=True, alignment=1, group=None, inequivalent_tokens=False, is_tight=False, Priority_Router=None):
    ...
    if Priority_Router is not None:
        topk_indices = Priority_Router.reroute(topk_indices)
    ...
    if is_tight:
        local_expert_count = torch.max((locations1 + 1 )* masks_se[0], dim=0)[0]
    ...
    if top_k > 1:
        ...
        for k in range(1, top_k):
            ...
            if is_tight:
                local_expert_count += torch.max((locations2 + 1)* masks_se[k], dim=0)[0]
            locations2 += acc_base
    ...
    if is_tight:
        ...
        offset = torch.zeros(num_global_experts, device=scores.device, dtype=torch.int)

        if capacity_factor != 0:
            capacity = top_k * int(capacity_factor * samples_per_expert)
            local_expert_count = torch.where(local_expert_count > capacity, capacity, local_expert_count)
        else:
            capacity = top_k * int(1 * samples_per_expert)

        offset[1:] = local_expert_count[:-1]
        offset = torch.cumsum(offset, dim=0)
        offset_s = [offset[idx] for idx in indices_s]

        return (num_global_experts, offset_s, local_expert_count, locations_s, gates_s, capacity), l_loss 
    return (num_global_experts, indices_s, locations_s, gates_s, capacity), l_loss

def fast_encode(data, critial_data, is_postscore=True, is_tight=False, cf0=False):
    assert not cf0 or is_tight, "cf0 is only valid when is_tight is True"
    num_global_experts = critial_data[0]
    dispatcher = TutelMoeFastDispatcher(num_global_experts, 0, data.size(-1), data.dtype, cf0)
    dispatcher.update(*critial_data[1:], is_postscore=is_postscore, is_tight=is_tight)
    return dispatcher.encode(data).view(num_global_experts, -1, data.size(-1))

def fast_decode(data, critial_data, is_postscore=True, is_tight=False, cf0=False):
    assert not cf0 or is_tight, "cf0 is only valid when is_tight is True"
    num_global_experts = critial_data[0]
    dispatcher = TutelMoeFastDispatcher(num_global_experts, 0, data.size(-1), data.dtype, cf0)
    dispatcher.update(*critial_data[1:], is_postscore=is_postscore, is_tight=is_tight)
